...
test.describe('Visual baselines — cancel + refund flow', () => {
  ...
  test('cancelled order banner — ORD-00450', async ({ page }) => {
    ...
    await expect(banner).toHaveScreenshot('order-cancelled-banner.png', {
      animations: 'disabled',
      // Tolerate sub-percent font-rendering / anti-aliasing noise. The
      // banner copy and layout are static (formatDateTime + cancellation
      // reason), so anything beyond this threshold is a real regression.
