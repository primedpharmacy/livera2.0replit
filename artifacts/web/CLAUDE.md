...
**Branch:** `wave-9-owner-dashboard` (Wave 9 COMPLETE)

**Last completed:** Owner Dashboard full rebuild — greeting + 6-stat strip + 3-col grid + activity feed ✅

**Overall completion: ~52%** (Wave 9 done — dashboard now fully prototype-faithful)
...
|---|---|---|
| BLD-DASH-01 | Owner Dashboard — greeting + mood pill + 6-stat strip | ✅ Done |
| BLD-DASH-02 | Owner Dashboard — 3-column grid (clinical work, complaints, incidents, Rx expiry, MHRA, flags, audit) | ✅ Done |
| BLD-DASH-03 | Owner Dashboard — full-width activity feed + real data wiring | ✅ Done |

**Wave 10 candidates (next priority):**
1. BLD-15.1 — Queue age indicator on clinical check queue rows
2. BLD-14.2 — 3-gate enforcement (approve button disabled if gates incomplete)
3. BLD-15.2 — ED safeguarding banner in order detail
4. BLD-YC-01 — MHRA Yellow Card panel in incident detail
5. Settings sub-pages (notifications, SLA config, team members)
6. KPI Dashboard (BLD-12.x) — stub only, Wave 12 content

**Key fixtures added in Wave 8:**
- `fixtures/tasks.ts` — 11 mock tasks (BLD-13.2)
- `fixtures/welcomeCalls.ts` — 3 canonical welcome calls (BLD-13.3)
- `fixtures/patients.ts` — added Michelle Clarke (PT-00210), Sarah Chen (PT-00214), Beth Newman (PT-00199)
- `fixtures/orders.ts` — SARAH_ORDER_FEELTRU enriched with nice_checklist (5 items), dose_escalation_gate (5mg→7.5mg eligible), weight_history (5 readings 92.5→84.2 kg)
...
│       ├── tasks/                  # Tasks list + detail (BLD-13.2 ✅)
│       ├── welcome-calls/          # Welcome call queue + detail (BLD-13.3 ✅)
...
│   │       ├── tasks.ts            # MOCK_TASKS, listTasks, getTask (BLD-13.2 ✅)
│   │       └── welcomeCalls.ts     # MOCK_WELCOME_CALLS, listWelcomeCalls, getWelcomeCall (BLD-13.3 ✅)
...
| Owner Dashboard | 95% | Activity feed links not wired (future) |
