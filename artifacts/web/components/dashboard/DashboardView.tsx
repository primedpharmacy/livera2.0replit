...
export function DashboardView({
  ...
  // Task-60 — new patient intake submissions awaiting clinical review.
  // Identified by the ORD-INTAKE- id prefix set by createIntakeOrder.
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-bdr rounded-lg overflow-hidden">
            ...
            <div className={`grid gap-0 p-3 ${clinicId === "feeltru" ? "grid-cols-5" : "grid-cols-4"}`}>
              {[
                ...
                ...(clinicId === "feeltru"
                  ? [{ href: "/feeltru/intake", Icon: Users, label: "Patient intake" }]
                  : []),
              ].map(({ href, Icon, label }) => (
                <Link
                  key={label}
          ...
          <div className="bg-surface border border-bdr rounded-lg overflow-hidden">
            <CardHeader
              ...
              badge={intakeOrders.length > 0 ? `${intakeOrders.length} new intake${intakeOrders.length > 1 ? "s" : ""}` : undefined}
              badgeColor={intakeOrders.length > 0 ? "bg-info-bg text-info border border-info-bdr" : undefined}
              href={`/${clinicId}/clinical-check`}
            ...
            {topCheckOrders.length === 0 ? (
              ...
              <div className="divide-y divide-bdr">
                {topCheckOrders.map((order) => {
                  ...
                  const isIntake = order.id.startsWith("ORD-INTAKE-");
                  return (
                    <Link
                      key={order.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[12px] font-semibold text-t1 truncate">{name}</span>
                          {isIntake && (
                            <span className="text-[9px] font-bold px-1.5 py-px rounded bg-info-bg text-info border border-info-bdr uppercase tracking-wide shrink-0">
                              New intake
                            </span>
                          )}
                        </div>
