...
**Branch:** `wave-7-bld-yc-01` (in progress — Wave 7 Gap-Close)

**Last completed:** BLD-YC-01 — MHRA Yellow Card panel on incident detail ✅

**Overall completion: ~47%** (BLD-YC-01 done 2026-05-13)

**Next priority — Wave 7 Gap-Close** (all have completed prototypes):

| BLD | Description | Size | Status | Prototype URL |
|---|---|---|---|---|
| ~~BLD-YC-01~~ | ~~MHRA Yellow Card panel in incident detail~~ | S | ✅ Done | `livera_scr_incident_detail_intercom` |
| BLD-INT-MHRA-02 | Intercom tag-actions settings update | S | Next | `livera_scr_intercom_webhook_config` |
| BLD-INT-MHRA-03 | MHRA rollup card on Owner Dashboard | S | `livera_owner_dashboard` |
| BLD-INTERCOM-PHOTO-01 | Photo attachments in patient Intercom tab | S | `livera_prototype_09_patient_profile` |
| BLD-INT-MHRA-01 | MHRA gov.uk alerts settings screen | M | `livera_settings_landing` |
| BLD-CALENDLY-MIRROR-01 | Calendly bookings mirror in Coaching tab | M | `livera_prototype_09_patient_profile` |

**After Wave 7 — Priority order:**
1. Tasks list + detail (BLD-13.2) — entire surface, 0% built
2. Owner Dashboard rebuild (BLD-INT-MHRA-03 + full content)
...
5. KPI Dashboard (BLD-12.x) — stub only
6. G6 Flag Dashboard (BLD-16.6) — stub only
