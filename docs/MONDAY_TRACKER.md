...
| BLD-INT-MHRA-03 | BLD-INT-MHRA-03 · Owner Dashboard MHRA alerts rollup card | S | Done | Proto done |
| BLD-INTERCOM-PHOTO-01 | BLD-INTERCOM-PHOTO-01 · Inline Intercom photo preview + atta | S | Done | Proto done |
| BLD-YC-01 | BLD-YC-01 · MHRA Yellow Card prompt inside incident detail | S | Done | Proto done |
| WAVE-6.5-PATIENT-PROFILE | Wave 6.5 · Patient Profile Visual Scaffold (LOCKED 13 May 20 | M | Done | Done |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-3.1 | Intervention 7-working-day SLA timer + breach auto-flag | M | Done | Done |
| BLD-3.2 | GP letter 48-hour SLA timer from approval + breach auto-flag | M | Done | Done |
| BLD-3.3 | Order Expiry auto-transition at 6 calendar days | M | Done | Done |
| BLD-3.4 | Expired tab on Orders Queue (SCR-009) | S | Done | Done |
| BLD-3.5 | On expiry: release Ryft auth + Omnisend template + log event | M | Done | Done |
| BLD-3.6 | Four-scenario dispatch date calculator | M | Done | Done |
| BLD-3.7 | UK public holiday calendar — Settings sub-screen | M | Done | Done |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-1.1 | Add coaching_enabled + gender_eligibility flags to clinic_co | S | Done | Done |
| BLD-1.2 | Set VSC amendment_window to pre_approval (V1.1 correction) | S | Done | Done |
| BLD-1.3 | Add reply_email + monday_incident_board_id to clinic_config | S | Done | Done |
| BLD-1.4 | Add default_slas object (10 SLA values) to clinic_config | S | Done | Done |
| BLD-1.5 | Remove Manager, Pharmacist, Technician role cards from SCR-0 | S | Done | Done |
| BLD-1.6 | Add Mobeen Alam as second Owner on FeelTru workspace | M | Done | Done |
...
| BLD-14.2 | Three-gate Clinical Check sequence enforcement | M | Done | Proto done |
| BLD-14.3 | NICE CG189 checklist screen in order approval | M | Done | Proto done |
| BLD-14.4 | Dose escalation gate + history panel | M | Done | Proto done |
| BLD-14.5 | Weight trajectory + check-in panel in order review | M | Done | Proto done |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-2.1 | Coach role + RBAC + Coach-specific sidebar nav | M | Done | Done |
| BLD-2.2 | Build Coach Dashboard — SCR-040 | M | Done | Done |
| BLD-2.3 | Coach login routing → SCR-040 as home screen | S | Done | Done |
| BLD-2.4 | Build coaching_log entity — 16 fields | M | Done | Done |
| BLD-2.5 | Coaching Log entry form modal | M | Done | Done |
| BLD-2.6 | Patient Profile — Coaching Log tab (FeelTru only) | M | Done | Done |
| BLD-2.7 | Clinical Escalation flag entity + workflow | M | Done | Done |
| BLD-2.8 | Escalation flag count on Clinician Home priority banner | S | Done | Done |
| BLD-2.9 | Latest Coaching Log card on re-order Clinical Check (FeelTru | M | Done | Done |

## 💳 Chunk 5 — Pre-Approval Refund Alignment (P0)

| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-5.1 | Hide pre_dispensed buttons in SCR-010 (VSC workspace) | S | Done | Done |
| BLD-5.2 | Update Amendment Window panel copy — V1.1 informational | S | Done | Done |
| BLD-5.3 | Verify pre_approval enforcement (no post-approval auto-cance | S | Done | Done |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-15.1 | Queue age indicator column | S | Done | Proto done |
| BLD-15.2 | Eating disorder safeguarding flag | M | Done | Not started |
...
| BLD-AMEND-01 | BLD-AMEND-01 — Amendment Request Queue | M | Done | — |
| BLD-AMEND-02 | BLD-AMEND-02 — Amendment Decision screen | M | Done | — |
| BLD-AMEND-03 | BLD-AMEND-03 — Amendments tab on order detail | S | Done | — |
| BLD-AMEND-04 | BLD-AMEND-04 — Payment delta state surface | S | Done | — |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-4.1 | admin_note entity — 9 fields | S | Done | Done |
| BLD-4.2 | Admin Note entry form modal (Patient Profile FAB) | M | Done | Done |
| BLD-4.3 | Unified Notes timeline — Clinical + Admin + Coaching Log | M | Done | Done |
| BLD-4.4 | Remove 'General' option from clinical note authoring UI | S | Done | Done |
| BLD-4.5 | General-type migration — deprecated_general_type flag | S | Done | Done |
...
| BLD-9.1 | complaint entity — 20 fields (CMP-XXXX, SLA timers, Monday I | M | Done | Done |
| BLD-9.2 | Complaints Log — SCR-041 (mirrors SCR-012 Incident Log) | M | Done | Done |
| BLD-9.3 | Complaint Detail — SCR-042 (mirrors SCR-013; dual SLA bar) | M | Done | Done |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-7.1 | Replace single GP letter template with two: Email Body + PDF | M | Done | Done |
| BLD-7.2 | Server-side PDF generation (headless Chromium / library) | L | Done | Done |
| BLD-7.3 | Postmark send — email body + PDF attachment in single messag | M | Done | Done |
| BLD-7.4 | Activity log captures email body + PDF filename + Postmark c | S | Done | Done |
| BLD-7.5 | SCR-015 compose modal — email body (left) + PDF preview (rig | M | Done | Done |
| BLD-7.6 | GP Letter Templates — per-clinic library (Settings) | M | Done | Done |
| BLD-7.7 | GP Letters list — patient-centric lifecycle workflow | M-L | Done | Done |
...
| BLD-13.2 | Task management feature — admin + clinician tasks | L | Done | Proto done |
| BLD-13.3 | Welcome call queue + Intercom phone integration UI | L | Done | Proto done |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-8.1 | Add intercom_thread_url + incident_origin to incident entity | S | Done | Done |
| BLD-8.2 | Configure Intercom 'Incident' tag + webhook subscription | M | Done | Done |
| BLD-8.3 | Webhook handler: resolve patient_id from Intercom → create I | M | Done | Done |
| BLD-8.4 | Closure rule: tagged Intercom thread cannot close until inci | M | Done | Done |
...
| BLD ID | Name | Complexity | Status | Status (ext) |
|--------|------|------------|--------|--------------|
| BLD-6.1 | Add AI audit trail fields to clinical_note entity | S | Done | Done |
| BLD-6.2 | Wire SCR-030 Approval Modal to claude-sonnet-4-20250514 | L | Done | Done |
| BLD-6.3 | Decline Confirm Modal + Intervention Confirm Modal | M | Done | Done |
| BLD-6.4 | Save audit trail on submit — original + edits + final | M | Done | Done |
| BLD-6.5 | AI note drafting system prompt — draft + Qadir sign-off | M | Done | Done |
...
| BLD-FLAG-1 | BLD-FLAG-1 — Rename + reframe pass (Phase 1, COMPLETE) | S | Done | Proto done |
...
| BLD-POLISH | Polish pass — stale dates + audit log nav | S | Done | Proto done |
...
| BLD-WC-DETAIL | Welcome Call detail screen | M | Done | Proto done |
