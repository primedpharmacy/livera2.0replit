...
// Task-314 — the HMAC primitives + dev-fallback secret live in
// `sessionSignature.ts` so middleware, route handlers, server actions,
// unit tests, and Playwright specs all share a single source of truth.
// Re-exported here so the existing `@/lib/auth/session` import path
// keeps working for the dozens of call sites that already use it.
export {
  SESSION_COOKIE_NAME,
  mintSessionCookieValue,
  verifySessionCookie,
