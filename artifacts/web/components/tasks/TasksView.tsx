...
import { Search, Plus, ChevronDown, X, CheckSquare } from "lucide-react";
import { cn } from "@/lib/utils";
import { NOW } from "@/lib/api/constants";
import { createTask } from "@/lib/api/mock";
import type { Task, TaskStatus, TaskPriority, ClinicTeamMember, ClinicId } from "@/types";
...
export function TasksView({ clinicId, tasks, members }: Props) {
  ...
  const [ntPriority, setNtPriority] = useState<TaskPriority>("med");
  const [ntDue, setNtDue]           = useState("");
  const [ntBusy, setNtBusy]         = useState(false);
  const [toast, setToast]           = useState<string | null>(null);

  function openNewTask() {
    setNtTitle(""); setNtDesc(""); setNtPriority("med");
    const d = new Date(NOW); d.setUTCDate(d.getUTCDate() + 1);
    setNtDue(d.toISOString().slice(0, 10));
    setShowNewTask(true);
  }

  function closeNewTask() { setShowNewTask(false); }

  async function submitNewTask() {
    if (!ntTitle.trim()) return;
    setNtBusy(true);
    await createTask(clinicId as ClinicId, { title: ntTitle.trim(), description: ntDesc.trim() || undefined, priority: ntPriority, due_date: ntDue || TODAY });
    setNtBusy(false);
    closeNewTask();
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-surface shrink-0">
        <div>
          ...
          <span className="ml-2 text-[12px] text-t3">
            {tasks.length} total · {open.filter(t => t.status !== "done").length} active
            {overdueCt > 0 && <> · <span className="text-err font-semibold">{overdueCt} overdue</span></>}
          </span>
        </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-[15px] font-semibold text-t1">New task</h3>
              </div>
              <button onClick={closeNewTask} className="text-t3 hover:text-t1 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-border rounded-md px-3 py-2 text-[13px] focus:outline-none focus:ring-1 focus:ring-brand/40"
                  placeholder="e.g. Chase pharmacist on dose change"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-border rounded-md px-3 py-2 text-[13px] focus:outline-none focus:ring-1 focus:ring-brand/40 resize-none"
                  rows={3}
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="text-[11px] font-semibold text-t2 uppercase tracking-wide block mb-1">Priority</label>
                    ...
                    {PRIORITIES.map((p) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex-1 flex items-center justify-center gap-1 text-[11px] font-semibold py-1.5 rounded-md border transition-colors",
                            ntPriority === p
                              ? `${cfg.text} border-current bg-current/10`
                              : "text-t3 border-border hover:bg-surface-2"
                          )}
                        >
                ...
                <div className="w-36">
                  <label className="text-[11px] font-semibold text-t2 uppercase tracking-wide block mb-1">Due date</label>
                  <input
                    type="date"
                    value={ntDue}
                    onChange={(e) => setNtDue(e.target.value)}
                    className="w-full border border-border rounded-md px-2.5 py-1.5 text-[12px] focus:outline-none focus:ring-1 focus:ring-brand/40"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 mt-5">
              ...
              <button
                ...
                disabled={!ntTitle.trim() || ntBusy}
                className="text-[12px] font-semibold text-white bg-brand rounded-md px-4 py-2 hover:bg-brand/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                {ntBusy ? "Creating..." : "Create task"}
              </button>
            </div>
          </div>
        </div>
