...
export function ClinicalCheckInboxEditor({ config, clinicId, actorId }: Props) {
  ...
  const [lastUpdate, setLastUpdate] = useState(() =>
    getLastClinicFieldUpdate(clinicId, "clinical_check_inbox"),
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      setBaseline(trimmed);
      setSaved(true);
      setLastUpdate(getLastClinicFieldUpdate(clinicId, "clinical_check_inbox"));
  ...
  return (
    <div className="bg-surface border border-bdr rounded-lg overflow-hidden">
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        {lastUpdate && (
          <p className="text-[11px] text-t3" data-testid="clinical-check-inbox-last-updated">
            Last updated by{" "}
