    ...
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (focusedIdx >= 0 && focusedIdx < filtered.length) {
          e.preventDefault();
          router.push(`/${clinicId}/incidents/${filtered[focusedIdx].id}`);
        }
      }
    }
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [filtered, focusedIdx, router, clinicId, showCreate]);
      ...
      <div className="px-6 py-4">
        {filtered.length === 0 ? (
          ...
          <div className="bg-surface border border-bdr rounded-lg overflow-hidden">
            {/* Legend */}
            <div className="flex items-center justify-between px-4 py-2 border-b border-bdr bg-surface">
