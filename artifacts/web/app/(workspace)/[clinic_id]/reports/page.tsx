        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <AuditCard
              ...
              badge="LIVE"
              bld="BLD-12.4"
              href={`/${clinic_id}/reports/aud-03`}
            />
            <AuditCard
              icon={<TrendingUp className="w-4 h-4" />}
              code="AUD-04"
              title="Patient Outcomes"
              description="Cohort outcomes audit · weight loss vs NICE CG189 5% target at 12 weeks · FeelTru & VSC cohort views · coaching impact analysis."
              badge="LIVE"
              bld="BLD-12.5"
              href={`/${clinic_id}/reports/aud-04`}
            />
            <AuditCard
...
function AuditCard({
  icon, code, title, description, badge, lastRun, bld, href,
}: {
  icon: React.ReactNode; code: string; title: string; description: string;
  badge: "MONDAY" | "COMING" | "LIVE"; lastRun?: string; bld: string; href?: string;
}) {
  const isMonday = badge === "MONDAY";
  const isLive   = badge === "LIVE";
  const iconCls  = isMonday || isLive ? "bg-ok-bg text-ok border border-ok-bdr" : "bg-page-bg text-t3 border border-bdr";
  const badgeCls = isMonday || isLive ? "bg-ok-bg text-ok border-ok-bdr"        : "bg-page-bg text-t3 border-bdr";
  return (
    <div className="bg-surface border border-bdr rounded-xl p-5 flex flex-col">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${badgeCls}`}>
          {badge}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        {isMonday && lastRun ? (
          ...
          <Link href={href} className="flex items-center gap-1 text-[12px] font-semibold text-brand hover:underline">
