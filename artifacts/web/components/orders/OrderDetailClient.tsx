            ...
            <DCard icon={ClipboardList} title="Questionnaire Responses">
              {Object.entries(order.questionnaire_responses).length === 0 ? (
                ...
                <div className="space-y-2">
                  {Object.entries(order.questionnaire_responses).map(([key, val]) => (
                    <div key={key} className="flex justify-between gap-4 py-1 border-b border-bdr last:border-0">
                      <span className="text-[12px] text-t3 capitalize">{key.replace(/_/g, " ")}</span>
                      <span className="text-[12px] text-t1 font-medium text-right">{String(val)}</span>
                    </div>
                  ))}
                </div>
              )}
