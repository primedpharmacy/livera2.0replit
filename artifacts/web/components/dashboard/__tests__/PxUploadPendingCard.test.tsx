...
// ── Task-274 — bulk "Send reminder to selected" ────────────────────────────
describe('PxUploadPendingCard — Task-274 bulk reminder', () => {
  afterEach(() => {
    cleanup();
    mockedResend.mockReset();
    global.fetch = ORIGINAL_FETCH;
  ...
  it('disables the checkbox on ineligible rows and hides the bulk action until something is selected', () => {
    const eligible  = makeOrder('ORD-FRESH', 'PT-FRESH', freshLink());
    const ineligible = makeOrder('ORD-EXPIRED', 'PT-EXPIRED', expiredLink());
    ...
    expect(screen.queryByTestId('px-upload-bulk-reminder')).toBeNull();

    const checkboxes = screen.getAllByTestId('px-upload-row-select');
    expect(checkboxes).toHaveLength(2);
    expect(checkboxes[0]).not.toBeDisabled();
    expect(checkboxes[1]).toBeDisabled();
  });

  it('POSTs the reminder route once per selected row and summarises successes/failures/skips in the toast', async () => {
    const orderA = makeOrder('ORD-A', 'PT-FRESH', freshLink());
    const orderB = makeOrder('ORD-B', 'PT-STALE', staleLink());
    const orderC = makeOrder('ORD-C', 'PT-RESEND', multiResendLink());
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ok: false,
        status: 502,
        json: async () => ({ message: 'Postmark down', status: 'Failed' }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
    ...
    const bulkButton = screen.getByTestId('px-upload-bulk-reminder');
    expect(bulkButton).toHaveTextContent('Send reminder to selected (3)');
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1, '/api/orders/feeltru/ORD-A/px-upload-reminder', { method: 'POST' },
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2, '/api/orders/feeltru/ORD-B/px-upload-reminder', { method: 'POST' },
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      3, '/api/orders/feeltru/ORD-C/px-upload-reminder', { method: 'POST' },
    );

    const toast = await screen.findByRole('status');
    expect(toast).toHaveTextContent('2 sent');
    expect(toast).toHaveTextContent('1 failed');

    // Selection clears after the bulk run completes.
    await waitFor(() => {
      expect(screen.queryByTestId('px-upload-bulk-reminder')).toBeNull();
    });
  });

  it('skips selected rows that became ineligible and reports them in the toast without POSTing', async () => {
    // Row whose link has already had both reminders sent — selectable only
    // if the checkbox guard slips, but the bulk handler must still skip.
    const doneLink: NonNullable<Order['px_upload_link']> = {
      ...freshLink(),
      reminder_sent_at: '2026-05-10T08:00:00Z',
      final_reminder_sent_at: '2026-05-11T07:00:00Z',
    };
    const eligible = makeOrder('ORD-A', 'PT-FRESH', freshLink());
    const done     = makeOrder('ORD-DONE', 'PT-STALE', doneLink);
    ...
    const fetchMock = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        kind: 'first',
        status: 'Delivered',
        px_upload_link: { ...freshLink(), reminder_sent_at: '2026-05-11T08:05:00Z' },
      }),
    });
    global.fetch = fetchMock as unknown as typeof fetch;
    ...
    render(
      <PxUploadPendingCard
        clinicId="feeltru"
        orders={[eligible, done]}
        patientMap={PATIENT_MAP}
      />,
    );

    const checkboxes = screen.getAllByTestId('px-upload-row-select');
    // "done" row's checkbox is disabled, so we only select the eligible row.
    expect(checkboxes[1]).toBeDisabled();
    fireEvent.click(checkboxes[0]);

    fireEvent.click(screen.getByTestId('px-upload-bulk-reminder'));

    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledTimes(1);
    });
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/orders/feeltru/ORD-A/px-upload-reminder',
      { method: 'POST' },
    );

    const toast = await screen.findByRole('status');
    expect(toast).toHaveTextContent('1 sent');
  });
});
