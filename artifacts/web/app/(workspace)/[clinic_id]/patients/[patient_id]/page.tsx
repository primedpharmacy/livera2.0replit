...
export default async function PatientProfilePage({ params, searchParams }: PatientProfilePageProps) {
  ...
  return (
    <Suspense key={`${clinic_id}-${patient_id}-${tab}`} fallback={<LoadingState.Detail />}>
      <ProfileContent
...
async function ProfileContent({
  ...
  try {
    ...
    if (coachingEnabled) {
      fetches.push(listCoachingLogs(clinicId, { patient_id: patientId }));
    }
...
// ── Profile body (existing inline content, extracted to sub-component) ────────
